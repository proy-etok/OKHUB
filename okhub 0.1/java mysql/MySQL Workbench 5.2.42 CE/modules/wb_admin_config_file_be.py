...
from wb_common import OperationCancelledError, InvalidPasswordError, PermissionDeniedError, splitpath
    ...
    if 'values' in opt:
        ...
        for cur_value in opt['values']:
            ...
            if inversion is None:
                ...
                inversion = parse_version_str(inversion)

            if outversion is None:
                outversion = (99,0,0)
            else:
                outversion = parse_version_str(outversion)
        ...
        if self.accept_action is not None:
            ...
            while True:
                try:
                    ...
                    r = Utilities.show_error("Could not Save Configuration File", "There was a permission error while saving the configuration file: %s\nClick Retry to enter the password again.",
                                  "Retry", "Cancel", "")
...
class WbAdminConfigFileBE(object):
    ...
    def reload_possible_options(self):
        ...
        if not server_version or str(server_version) == '':
            ...
            log_debug2('%s:%s.reload_possible_options()' % (_this_file, self.__class__.__name__) ,
                       'Got server version "%s" from the server\n' % str(server_version))
        else:
            print "Note! Workbench uses server version '%s' from the server instance profile. Make the entry empty to auto pick version from the server." % (str(server_version))
    ...
    def transform_opts_for(self, server_version, platform):
        log_debug2('%s:%s.transform_opts_for()' % (_this_file, self.__class__.__name__) , 'Filtering options for %s, "%s"\n' % (str(server_version), platform) )
            ...
            for (grpname, grpcont) in tabcont:
                ...
                for opt in grpcont:
                    ...
                    if value:
                        ...
                        log_debug('%s:%s.transform_opts_for()' % (_this_file, self.__class__.__name__) ,
                                  'Option "%s" skipped because of missing value\n' % opt['name'])
                        no_value += 1
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                ...
                Utilities.show_warning("Read Configuration File",
                                       "Configuration file '%s' can not be found. New file will be created on apply of changes."%file_name,
              "OK", "", "")
    ...
    def read_mysql_cfg_file(self, file_name):
        ...
        log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__),
                  'Reading config file "%s"\n' % file_name)
        content = None
        if not self.needs_root_for_file_read:
            try:
                log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__),
                          'Trying to read without sudo\n')
                content = self.ctrl_be.server_helper.get_file_content(file_name, as_admin=False, admin_password=None)
                log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__),
                          '%i bytes read from file\n' % len(content or []) )
            except PermissionDeniedError, e:
                log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__),
                          'Permissin denied, sudo needed to read config file: "%r"\n' % e)
                self.needs_root_for_file_read = True

        if self.needs_root_for_file_read:
            log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__),
                      'Trying to read with sudo\n')
            admin_password = self.ctrl_be.password_handler.get_password_for("file")
            as_admin = admin_password is not None
            try:
                log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__), 'Reading...\n')
                content = self.ctrl_be.server_helper.get_file_content(file_name, as_admin, admin_password)
            except InvalidPasswordError, err:
                log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__), 'Invalid password error: "%r"\n' % err)
                self.ctrl_be.password_handler.reset_password_for("file")
                raise err

        log_debug('%s:%s.read_mysql_cfg_file()' % (_this_file, self.__class__.__name__), 'Config file read\n')
    ...
    def save_config_file(self, user_modified_file_content):
        ...
        if not helper.is_dir_writable(directory):
            password = self.ctrl_be.password_handler.get_password_for("file")
        else:
            password = None

        try:
            helper.set_file_content_and_backup(self.file_name, user_modified_file_content, ".wba.bak", password is not None, password)
        except InvalidPasswordError, err:
            self.ctrl_be.password_handler.reset_password_for("file")
            raise err
    ...
    def parse_file_contents(self, file_data = None):
        ...
        log_debug('%s:%s.parse_file_contents()' % (_this_file, self.__class__.__name__),
                  'Parsing options only from section "%s"\n' % filter_by_section)
    ...
    def option_added(self, name, value, section):
        log_debug('%s:%s.option_added()' % (_this_file, self.__class__.__name__),
                  'Adding option: "%s", "%r", "%s"\n' % (name, value, section) )
        if value is None:
            value = True
        ...
        if option_type == 'boolean':
            ...
            log_debug3('%s:%s.option_added()' % (_this_file, self.__class__.__name__),
                'adding boolean option "%s" with value = "%r", default = "%r", orig_opt = "%s", disabledby = "%r", definition = "%s"\n' % (name, value, option_default, orig_opt, disabledby, odef) )
            ...
            if disabledby is not None:
                if disabledby in self.original_opts:
                    ...
                    log_debug3('%s:%s.option_added()' % (_this_file, self.__class__.__name__), 'added delete ci for "%s"\n' % name)

            if orig_opt is None:
                if name in self.changeset:
                    del self.changeset[name]
                    log_debug3('%s:%s.option_added()' % (_this_file, self.__class__.__name__), 'Removing existing changeset for "%s"\n' % name)
                ...
                if option_default != True:
                    ...
                    log_debug3('%s:%s.option_added()' % (_this_file, self.__class__.__name__), 'Adding ci=ADD for "%s" with value "%s"\n' % (name, str(ci.value)) )
                    self.changeset[name] = ci
            else:
    ...
    def option_removed(self, name, section):
        log_debug('%s:%s.option_removed()' % (_this_file, self.__class__.__name__), 'opt removed "%s"\n' % name)
        ...
        if option_type == 'boolean':
            ...
            log_debug3('%s:%s.option_removed()' % (_this_file, self.__class__.__name__),
                       'removing "%s", orig_opt = "%s", off_value = "%r", odef = "%s"\n' % (name, orig_opt, off_value, odef) )
    ...
    def option_changed(self, name, value, section):
        log_debug('%s:%s.option_changed()' % (_this_file, self.__class__.__name__), 'Option changed: "%s", "%r"\n' % (name, value) )
    ...
    def apply_changes(self):
        ...
        for c in change:
            ...
            if c.mod == CHANGE:
                log_debug('%s:%s.apply_changes()' % (_this_file, self.__class__.__name__), 'Applying change "%r"\n' % c)
                ...
                for i in range(min(orig_values_len, modified_values_len)):
                    ...
                    log_debug2('%s:%s.apply_changes()' % (_this_file, self.__class__.__name__), 'line_nr "%r", "%r"\n' % (line_nr, c.orig_opt) )
                    rvalue = "\n"
...
def init_cfg_be(ctx):
    ...
    # This will connect to mysql server and get version
