...
class WizardProgressPage(wizard_page_widget.WizardPage):
    ...
    def send_progress(self, pct, msg):
        self._handle_task_output("PROGRESS", "%f:%s"%(pct, msg), "")
    ...
    def _finished(self):
        ...
        if self._errors > 0:
            self._detail_label.set_text("There were errors during execution, please review log messages.")
            if not self._showing_logs:
                self.go_advanced()
            self.tasks_failed(False)
        elif self._warnings > 0:
            self._detail_label.set_text("There were warnings during execution, please review log messages.\nClick [Next >] to continue if you think they are not important.")
            if not self._showing_logs:
                self.go_advanced()
            self.tasks_failed(False)
        else:
    ...
    def _flush_messages(self):
        while len(self._log_queue) > 0:
            ...
            if mtype == "INFO":
                ...
                try:
                    self.update_progress(float(pct), text)
                except:
                    pass
            else:
                if mtype == "ERROR":
