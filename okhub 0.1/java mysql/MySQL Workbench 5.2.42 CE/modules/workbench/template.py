...
class MiniTemplate:
    ...
    def render_(self, data, i, result, context):
        ...
        while i < len(self._tokens):
            ...
            if debug:
                print "Process %s: %s, token: %s" % (self.t2l(i), ".".join(context), token.strip())
                    ...
                    if type(sub) is list:
                        ...
                        for j, item in enumerate(sub):
                            ...
                            if j < len(sub)-1:
                                ...
                                itemd["needsep"] = 0
                            if debug:
                                print "%s: entering lcontext %s at %s" % (self.t2l(i), ".".join(context + [token, str(j)]), i+1)
