...
from workbench.db_utils import QueryError, MySQLError
import wb_admin_ssh
...
class SqlDataSource(DataSource):
    ...
    def __init__(self, sql, mon_be):
        ...
        log_debug2('%s:%s.__init__()' % (_this_file, self.__class__.__name__), 'SqlDataSource created. poll_control = %i\n' %  self.poll_control)
    ...
    def server_started_event(self):
        ...
        log_debug2('%s:%s.server_started_event()' % (_this_file, self.__class__.__name__), 'SqlDataSource server started. poll_control = %i\n' %  self.poll_control)
    ...
    def server_stopped_event(self):
        ...
        self.dbconn = None
        self.poll_control = self.stopped
        log_debug2('%s:%s.server_stopped_event()' % (_this_file, self.__class__.__name__), 'SqlDataSource server stopped. poll_control = %i\n' %  self.poll_control)
    ...
    def shutdown_event(self):
        ...
        self.dbconn = None
        self.poll_control = self.stopped
        log_debug2('%s:%s.shutdown_event()' % (_this_file, self.__class__.__name__), 'SqlDataSource shutdown. poll_control = %i\n' %  self.poll_control)
    ...
    def poll(self):
        log_debug3('%s:%s.poll()' % (_this_file, self.__class__.__name__), 'SqlDataSource poll. poll_control = %i\n' %  self.poll_control)
...
class WBAWinRemoteStats(object):
    ...
    def reader(self, ssh_session):
        ...
        log_debug('%s:%s.reader()' % (_this_file, self.__class__.__name__),
                  'Leaving monitor thread which polls remote windows\n')
            ...
            if c == "\n":
                ...
                if len(line) == 0:
                    ...
                    if line[0] == 'C':
                        ...
                        self.mtx.release()
                    line = ""
                    log_debug2('%s:%s.read_from_script()' % (_this_file, self.__class__.__name__),
...
class WBAdminMonitorBE(object):
    ...
    def poll_sources(self):
        ...
        log_debug('%s:%s.poll_sources()' % (_this_file, self.__class__.__name__), 'Exiting monitor thread...\n')
