...
from workbench.db_utils import MySQLConnection, MySQLError, QueryError, ConnectionTunnel, escape_sql_string, escape_sql_identifier
...
class DumpThread(threading.Thread):
    ...
    def process_db(self, respipe, extra_arguments, object_names):
        ...
        try:
            ...
            log_debug(_this_file, "Executing command: %s\n" % logstr)
            ...
            if platform.system() != 'Windows':
                try:
                    ...
                    log_error(_this_file, "Error executing command %s\n%s\n" % (logstr, exc))
                    import traceback
                    traceback.print_exc()
...
#                    pass
            ...
            while p1.poll() == None and not self.abort_requested:
                err = p1.stderr.read()
                if err != "":
                    log_error(_this_file, "Error from task: %s\n" % err)
            ...
            import traceback
            traceback.print_exc()
            log_error(_this_file, "Error executing task: %s\n" % exc)
            self.print_log_message("Error executing task: %s" % str(exc))
#        finally:
#            pass
        if pwdfilename:
        ...
        exitcode = p1.poll()
        if exitcode != 0:
            log_warning(_this_file, "Task exited with code %s\n" % exitcode)
            self.print_log_message("Operation failed with exitcode " + str(exitcode))
        else:
            log_debug(_this_file, "Task exited with code %s\n" % exitcode)
            self.print_log_message("")

        if result:
            log_debug(_this_file, "Task output: %s\n" % result)
    ...
    def kill(self):
        ...
        if self.process_handle:
            if platform.system() == 'Windows':
                ...
                log_debug(_this_file, "Killing task: %s\n" % cmd)
                subprocess.Popen(cmd , shell=True)
            else:
                import signal
                try:
                    log_debug(_this_file, "Sending SIGTERM to task %s\n" % self.process_handle.pid)
                    os.kill(self.process_handle.pid, signal.SIGTERM)
                except OSError, exc:
                    log_error(_this_file, "Exception sending SIGTERM to task: %s\n" % exc)
...
class WbAdminImportTab(WbAdminSchemaListTab):
    ...
    def refresh_schema_list(self):
        ...
        Utilities.add_timeout(float(0.4), self._update_schema_list)
...
#                operations.append((title, count, lambda:self.dump_to_file([]), params, schema_names))
    ...
    def check_refresh(self):
        ...
        for row in range(self.schema_list.count()):
            node = self.schema_list.node_at_row(row)
            schemaname = row.get_string(1)
            if "1wb_export_test" == schemaname:
                schema_node = row
                break
        if schema_node is None:
            return "No schema found in list"
        schema_node.set_bool(0, True)
        self.schema_list.select_node(schema_node)
        self.schema_selected()
