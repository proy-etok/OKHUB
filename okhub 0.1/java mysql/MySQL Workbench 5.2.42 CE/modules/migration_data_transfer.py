...
class SetupMainView(WizardPage):
    ...
    def _add_script_checkbox_option(self, box, name, caption, path_caption, browser_caption):
        ...
        check.add_clicked_callback(lambda box=file_box, check=check: box.set_enabled(check.get_active()))
    ...
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "DATA MIGRATION", "Data Transfer Setup")
    ...
    def _cell_edited(self, node, column, value):
        if column == 1:
            ...
            if count is not None:
                ...
                if refs:
                    r = mforms.Utilities.show_warning("Limit Table Copy",
...
class TransferMainView(WizardProgressPage):
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "DATA MIGRATION", "Bulk Data Transfer")
    ...
    def _prepare_copy(self):
        ...
        for table in tables:
            ...
            for column in table.columns:
                ...
                if cast:
                    select_expression.append(cast.replace("?", source_db_module.quoteIdentifier(column.name)))
                else:
                    select_expression.append(source_db_module.quoteIdentifier(column.name))
        ...
        if sys.platform == "win32":
            ...
            for arg in self._transferer.helper_basic_arglist():
                if ';' in arg or ' ' in arg:
                    ...
                    f.write(' %s' % arg)
            f.write('  --source-password="%arg_source_password%" --target-password="%arg_target_password%" %arg_truncate_target% %arg_debug_output%')
        else:
