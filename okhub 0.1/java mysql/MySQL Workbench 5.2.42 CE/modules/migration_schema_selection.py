...
class SchemaMainView(WizardPage):
    ...
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self,  'SOURCE & TARGET', 'Schemata Selection')
        ...
        self.options[0].set_active(True)
    ...
    def page_activated(self, advancing):
        ...
        if advancing:
            ...
            if self.doesSupportCatalogs:
                catalog_schemata_list = [ (catalog_name, schema_name) for catalog_name, dot, schema_name in ( full_name.partition('.') 
                ...
                self.catalog_schemata = [ schema_name for catalog_name, dot, schema_name in (full_name.partition('.') 
                                            for full_name in self.main.plan.migrationSource.schemaNames) ]
                self._optionspanel.show(False)
...
so that its structure can be determined.""")
        ...
        self.main.add_wizard_page(self, "SOURCE & TARGET", "Reverse Engineer Source")
