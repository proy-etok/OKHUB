    ...
    for param in connection.driver.parameters:
        if param.paramType in ('keychain', 'password'):
            if param.paramType == 'keychain':
                ...
                username, storage_string = replace_string_parameters(param.name, connection_params).split('::', 1)
...
class SourceWizardPage(WizardPage):
    ...
    def test_connection(self, source, caption):
        ...
        if test_connectivity(self.panel.connection, "Test %s DBMS Connection" % caption) == False:
        ...
        while True:
            try:
                ...
                mforms.Utilities.show_message("Test %s DBMS Connection" % caption, "Connection succeded.", "OK", "", "")
                break
...
class SourceMainView(SourceWizardPage):
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "SOURCE & TARGET", "Source Selection")
...
class TargetMainView(SourceWizardPage):
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "SOURCE & TARGET", "Target Selection")
...
#        self.server_instance_box.add(self.just_script_choice, False)
    ...
    def go_advanced(self):
        self.main.plan.setTargetConnection(self.panel.connection)
        self.test_connection(self.main.plan.migrationSource, "Target")
...
class FetchProgressView(WizardProgressPage):
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "SOURCE & TARGET", "Fetch Schemata List")
