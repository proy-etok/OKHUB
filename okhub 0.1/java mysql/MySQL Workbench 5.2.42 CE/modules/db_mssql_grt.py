    ...
    if serverVersion.majorNumber >= 9:  # Alias can be used as table column datatypes from SQL Server 2005 onwards
        ...
        for name, base_type, length, precision, scale, is_nullable in execute_query(connection, query):
            ...
            if not actual_type_found:
                grt.send_warning('MSSQL reverseEngineerUserDatatypes', 'Could not found base type "%s" for user defined type "%s"' % (base_type, name) )
...
def reverseEngineerViews(connection, schema):
    ...
    query_post_90 = """SELECT OBJECT_NAME(object_id) as name, definition
FROM sys.sql_modules
WHERE OBJECT_SCHEMA_NAME(object_id) = '%s'"""
    query_pre_90 = """SELECT TABLE_NAME as name, VIEW_DEFINITION AS definition
FROM INFORMATION_SCHEMA.VIEWS
WHERE TABLE_SCHEMA='%s'"""
    ...
    if cursor:
        ...
        for idx, (view_name, view_definition) in enumerate(resultset):
...
#    views = getViewNames(connection, schema.owner.name, schema.name)
#    for view_name in views:
#        reverseEngineerView(connection, schema, view_name)
...
def reverseEngineerProcedures(connection, schema):
    ...
    execute_query(connection, 'USE %s' % schema.owner.name)  # catalog
    query_post_90 = """SELECT OBJECT_NAME(M.object_id) as name, definition
FROM sys.sql_modules M JOIN sys.procedures P ON M.object_id=P.object_id
WHERE OBJECT_SCHEMA_NAME(M.object_id) = '%s'"""
    query_pre_90 = """SELECT ROUTINE_NAME as name, ROUTINE_DEFINITION as definition
FROM INFORMATION_SCHEMA.ROUTINES
WHERE ROUTINE_SCHEMA='%s' AND ROUTINE_TYPE='PROCEDURE'"""
    ...
    if cursor:
        ...
        resultset = cursor.fetchall()
        step = 1.0 / (len(resultset) + 1e-10)
        for idx, (proc_name, proc_definition) in enumerate(resultset):
...
WHERE ROUTINE_SCHEMA=? AND ROUTINE_TYPE='FUNCTION'"""
    ...
    if cursor:
        ...
        resultset = cursor.fetchall()
        step = 1.0 / (len(resultset) + 1e-10)
        for idx, (func_name, func_definition) in enumerate(resultset):
