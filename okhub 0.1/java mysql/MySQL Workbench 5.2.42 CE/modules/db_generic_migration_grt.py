...
from workbench.utils import find_object_with_name, find_object_with_old_name
...
class GenericMigration(object):
    ...
    def migrateIdentifier(self, name, log):
        mysql_name = name
        ...
        if True:
            ...
            for column in targetTable.columns:
                if column.autoIncrement and column.name not in pk_cols:
                    ...
                    state.addMigrationLogEntry(0, sourceTable, targetTable,
    ...
    def secondary_default_value_validation(self, state, source_column, target_column):
        # Only 1 CURRENT_TIMESTAMP column can exist
            ...
            if target_column.simpleType.name == 'TIMESTAMP' and target_column.defaultValue == 'CURRENT_TIMESTAMP':
                for column in target_column.owner.columns:
                    if column == target_column:
                        break
                    if column.simpleType and column.simpleType.name == 'TIMESTAMP' and column.defaultValue == 'CURRENT_TIMESTAMP':
                        state.addMigrationLogEntry(1, source_column, target_column,
                              'Multiple columns with DEFAULT CURRENT_TIMESTAMP not supported')
    ...
    def migrateTableColumnToMySQL(self, state, source_column, targetTable):
        ...
        target_column.name = self.migrateIdentifier(source_column.name, log)
    ...
    def migrateTableIndexToMySQL(self, state, source_index, targetTable):
        ...
        target_index.name = self.migrateIdentifier(source_index.name, log)
    ...
    def migrateTableForeignKeyToMySQL(self, state, source_fk, targetTable):
        ...
        target_fk.name = self.migrateIdentifier(source_fk.name, log)
        ...
        for source_fk_column in source_fk.columns:
            try:
                ...
                if not target_fk_col:
                    raise ValueError, 'The column "%s" was not found in table "%s"' % (self.migrateIdentifier(source_fk_column.name, None), targetTable.name)
        ...
        for source_fk_column in source_fk.referencedColumns:
            try:
                target_fk_col = self.findMatchingTargetObject(state, source_fk_column)
                if not target_fk_col:
                    raise ValueError, 'The column "%s" was not found in table "%s"' % (self.migrateIdentifier(source_fk_column.name, None), target_fk.referencedTable.name)
