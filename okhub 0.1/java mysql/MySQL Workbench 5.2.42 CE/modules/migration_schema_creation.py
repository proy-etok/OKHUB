...
class MainView(WizardPage):
    ...
    def _browse_files(self):
        ...
        form.set_path(self._create_script_file.get_string_value())
        if form.run_modal():
            self._create_script_file.set_value(form.get_path())
            self._check_file_duplicate = False

    def go_next(self):
        self.main.plan.state.objectCreationParams["Keep"] = self._keep_schema.get_active()
...
you may correct them in the next step. Table data will be migrated at a later step.""")
    ...
    def create_ui(self):
        ...
        self._db_task3 = self.add_threaded_task(self._create_task, "Create Schemata and Objects")
...
class CreationReportView(WizardPage):
    def __init__(self, main):
        ...
        self.main.add_wizard_page(self, "OBJECT MIGRATION", "Create Target Results")
