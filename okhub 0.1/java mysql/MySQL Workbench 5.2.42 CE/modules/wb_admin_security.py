...
class SecurityAccount(mforms.Box):
    def __init__(self, owner):
        ...
        self.password_advice = 'Consider using a password with 8 or more characters combining upper and lower-case letters, numbers and punctuation marks.'
        self.password_label = add(dLabel(self.password_advice))
        table.add(self.password_label, 1, 3, 4, 5, mforms.HFillFlag|mforms.HExpandFlag)
        table.add(add(rLabel("Confirm Password:")), 0, 1, 5, 6, mforms.HFillFlag)
        table.add(self.confirm, 1, 2, 5, 6, mforms.HFillFlag)
        table.add(add(dLabel("Enter password again to confirm.")), 2, 3, 5, 6, mforms.HFillFlag|mforms.HExpandFlag)
        ...
        if self.has_extra_plugins:
            ...
            table.add(dLabel("Authentication plugin specific parameters."), 2, 3, 7, 8, mforms.HFillFlag)
