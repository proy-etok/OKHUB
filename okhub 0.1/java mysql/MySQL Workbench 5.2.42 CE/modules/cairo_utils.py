...
class Node:
    ...
    def stroke_line_to_parent(self, c, parent):
        if parent:
            c.set_line_width(2)
            c.set_source_rgba(0.6, 0.6, 0.6, 1)
            v1 = parent.bottom_vertex()
            v2 = self.top_vertex()
            p1 = intersect_lines(v1[0], v1[1], line_center(*v2), line_center(*v1))
            p2 = intersect_lines(v2[0], v2[1], line_center(*v2), line_center(*v1))
            if p1 and p2:
...
class VBoxNode(Node):
    ...
    def stroke_line_to_parent(self, c, node):
        ...
        p1, p2 = ch.stroke_line_to_parent(c, node)
        ch.pos = pos
        return p1, p2
